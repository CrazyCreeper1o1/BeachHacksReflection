...
public class TimerManager : MonoBehaviour
{
    ...
    public float TimeSinceStart(string _timerName)
    {
        if (Timers.ContainsKey(_timerName))
        {
            return (Time.time - Timers[_timerName][0]);
        }
        return 0f;
    }
    ...
    public bool AddTask(Action _timedAction, float _duration, out string _taskName)
    {
        ...
        Tasks.Add(_tempTaskName, new TaskPair(_localTimerName, _timedAction));

        return TaskAdded;
    }

    public bool AddTask(Action _timedAction, float _duration)
    {
        bool TaskAdded = false;
    ...
    void Update()
    {
        foreach (KeyValuePair<string,TaskPair> _keyValuePair in Tasks.ToArray())
        {
